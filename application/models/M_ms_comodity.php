...
class M_ms_comodity extends CI_Model {
	...
	{
		$data = $this->db->query("
				select ".implode(',', $aColumns).", as id_key  from admin.ms_comodity where 0=0 $sWhere $sOrder $sLimit
			")->result_array();
		return $data;
	}
	...
	{
		$data = $this->db->query("
				select ".implode(',', $aColumns).", as id_key  from admin.ms_comodity where 0=0 $sWhere
			")->num_rows();
		return $data;
	}
