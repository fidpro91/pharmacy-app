    <div class="col-md-12">
    	...
    	<div class="col-md-6">
    		...
    		<?= create_select2([
				"attr" => ["name" => "unit_id=unit peminta ", "id" => "unit_id", "class" => "form-control"],
    	...
    	<div class="col-md-6">
    		...
    		<?= create_select2([
				"attr" => ["name" => "unit_target=unit pengirim ", "id" => "unit_target", "class" => "form-control"],
				"model" => ["m_bon" => "get_unit", "column" => ["unit_id", "unit_name"]]
			]) ?>
    		<div class="form-group">
    ...
    <script type="text/javascript">
    	...
    	// 		// $(this).autocomplete({
    	// 		// 	source: "<?php echo site_url('bon/get_item'); ?>/" + $("#item_id").val(),
    	...
    	$("body").on("focus", ".autocom_item_id", function() {
    		$(this).autocomplete({
    			source: "<?php echo site_url('bon/get_item'); ?>",
    			select: function(event, ui) {
    				$(this).closest('tr').find('.item_id').val(ui.item.item_id);
    				$(this).closest('tr').find('.satuan').val(ui.item.item_unitofitem);
    				$(this).closest('tr').find('.Stock_Terkini').val(ui.item.total_stock);
    			}
    		}).data("ui-autocomplete")._renderItem = function(ul, item) {
    			return $("<li>")
    				.append("<div class='comment-text'><span class=\"username\"><b>" +
    					item.item_name + "|" + item.item_code +
    					"</b><span class=\"text-muted pull-right\">" + formatNumeric(item.harga) + "</span></span><p>" +
    					"<span>Satuan : <span class=\"text-muted pull-right\">" + (item.item_unitofitem) + "</span></span><br>" +
    					"<span>Kategori Item : <span class=\"text-muted pull-right\">" + (item.classification_name) + "</span></span><br>" +
    					"<span>Stok terakhir : <span class=\"text-muted pull-right\">" + formatNumeric(item.total_stock) + "</span></span>" +
    					"</div>")
    				.appendTo(ul);
