...
class Sale2 extends MY_Generator
{
	...
	public function index()
	{
		...
		$data['unit'] = $kat;
		$this->theme('sale2/index', $data, get_class($this));
	}
	...
	public function save2()
	{
		...
		$input['sale_total'] = $data["grand_total"];
		$input['sale_embalase'] = $data["pembulatan"];
		$input['embalase_item_sale'] = $data["embalase_item"];
		$input['sale_services'] = $data["biaya_racikan"];
		...
		$saleDetail = [];
		//nonracikan
		$nonRacikan = $this->set_item_nonracikan2();
		if ($nonRacikan["code"] == 200) {
			$nonRacikan['item'] = array_map(function ($arr) use ($saleId){
				return $arr + ['sale_id' => $saleId];
			}, $nonRacikan['item']);
			$saleDetail = $nonRacikan['item'];
		}
	...
	public function cetak_prb($sale_id) {
		...
		$diagnosa = explode("-",$prb->program_prb);
			...
			$html = $this->load->view('sale2/v_cetak_prb',$data);
	...
	public function show_form($id)
	{
		...
		$this->load->view("sale2/form_sale", $data);
	}
	...
	public function show_form_update($id)
	{
		...
		$data['sale_id'] = $id;
		$data['model'] 	 = $this->m_sale->rules();
		$this->load->view("sale2/form_update", $data);
	}

	public function show_history_px()
	{
		$this->load->view("sale2/v_history_patient");
	}
	...
	public function get_history_px()
	{
		...
		")->result();
		$this->load->view("sale2/v_list_history_px", $data);
	}
	...
	public function set_item_racikan($post = null)
	{
		...
		foreach ($post['list_item_racikan'] as $x => $v) {
			...
			$itemRacik[$x]['kronis'] = $post['kronis'];
			$itemRacik[$x]['own_id'] = $post['own_id'];
			$itemRacik[$x]['percent_profit'] = $post['margin_profit'];
			...
			$itemRacik[$x]['sale_price'] = ($v['sale_price'] / (1+$post['margin_profit']));;
			$itemRacik[$x]['subtotal'] = $v['price_total'];
	...
	public function set_item_nonracikan2($post = null)
	{
		if (!$post) {
			$post = $this->input->post();
		}
		if (!empty($post['list_obat_nonracikan'])) {
			foreach ($post['list_obat_nonracikan'] as $x => $v) {
				...
				$itemNonRacikan[$x]['kronis'] = $post['kronis'];
				$itemNonRacikan[$x]['own_id'] = $post['own_id'];
				$itemNonRacikan[$x]['racikan'] = 'f';
				$itemNonRacikan[$x]['percent_profit'] = $post['margin_profit'];
				$itemNonRacikan[$x]['sale_price'] 	  = ($v['sale_price'] / (1+$post['margin_profit']));
				$itemNonRacikan[$x]['subtotal'] = $v['price_total'];
			}
			$resp = [
				'code' 		=> 200,
				'item' 		=> $itemNonRacikan
	...
	public function get_own_surety($id,$surety_id)
	{
		$data = $this->db->join("farmasi.ownership ow", "ow.own_id=so.own_id")
				->join("yanmed.ms_surety mr","mr.surety_id = so.surety_id")
				->select("so.percent_profit, mr.surety_name")
				->order_by("surety_code","asc")
				->get_where('farmasi.surety_ownership so', [
					"so.own_id"		=> $id,
					"so.surety_id"	=> $surety_id,
				])->row();
		echo json_encode($data);
	}
	...
	public function strukapotikresep($sale_id, $unit_id, $type)
	{
		...
		if ($type == 1) {
			$mpdf = new \Mpdf\Mpdf();
			$html = $this->load->view('sale2/v_cetakanresep3', $data, true);
			$mpdf->WriteHTML($html);
			$mpdf->Output();
		} else {
			$this->load->view('sale2/v_cetakanresep2', $data);
		}
	}
	public function struketiket($sale_id)
	{
		...
		//print_r($data);die;
		//$mpdf = new \Mpdf\Mpdf();
		$this->load->view('sale2/v_cetakanetiket',$data);
