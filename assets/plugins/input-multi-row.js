(function ($) {
	...
	$.fn.inputMultiRow = function (options) {
		...
		var tb =
			'<button type="button" class="btn btn-info pull-right btnplus_' +
			nama +
			'"><i class ="fa fa-plus"></i></button><table class="table tb_' +
			nama +
			'" style="width:100%"><thead></thead><tbody></tbody></table';
		this.html(tb);
		var header = "<tr>";
		// options.column();
		...
		if (options.data) {
			$.each(options.data, function (key, objek) {
				...
				row +=
					'<td><button class="btn btn-danger btn-xs removeItem_' +
					nama +
